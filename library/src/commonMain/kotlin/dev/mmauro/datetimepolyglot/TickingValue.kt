 ...
 */
public data class TickingValue<out T>(val value: T, val nextTick: Duration?) {
    init {
        if (nextTick != null) {
 ...
 */
public fun <T> TickingValue<T>.withNextTickAtMost(nextTick: Duration?): TickingValue<T> = TickingValue(
    value = value,
