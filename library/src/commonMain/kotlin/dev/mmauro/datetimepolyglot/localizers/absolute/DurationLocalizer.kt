...
/**
 * @property minUnit minimum unit that will be in output. If the [Duration] is smaller, `0 <min-unit>` will be returned
...
internal fun List<Pair<Long, DurationUnit>>.filter(options: DurationOptions) = this
    .filter { (value) -> !options.omitZeros || value != 0L }
    .ifEmpty { listOf(last()) }

internal fun Duration.requireValidAbsoluteDuration() {
