...
class RelativeInstantLocalizerTest : FunSpec({
    ...
    context("smoke tests") {
        val localizer = RelativeInstantLocalizer(locale = LOCALE_ENGLISH)
        val instantA = Instant.parse("2026-04-05T10:00:00.000Z")
        val instantB = Instant.parse("2026-04-05T11:15:00.000Z")
        test("past") {
            instantA.localizeRelative(reference = instantB) shouldBeLocalizedAs TickingValue(
                value = "1 hour ago",
                nextTick = 45.minutes,
            )
        }
        test("now") {
            instantA.localizeRelative(reference = instantA) shouldBeLocalizedAs TickingValue(
                value = "now",
                nextTick = 1.seconds,
            )
        }
        test("future") {
            instantB.localizeRelative(reference = instantA) shouldBeLocalizedAs TickingValue(
                value = "in 1 hour",
